...
def load_model(cuda: bool, model_name: str = 'default') :
    ...
    if model_name == 'default' and INPAINTING_MODEL is None :
        model = AOTGenerator()
        sd = torch.load(path + 'inpainting.ckpt', map_location = 'cpu')
    ...
    if model_name == 'lama' and INPAINTING_MODEL is None :
        ...
        sd = torch.load(path + 'inpainting_lama.ckpt', map_location = 'cpu')
        model.load_state_dict(sd['model'] if 'model' in sd else sd)
        model.eval()
        if cuda :
            model = model.cuda()
    ...
    if model_name == 'lama_mpe' and INPAINTING_MODEL is None :
        model = load_lama_mpe(path + 'inpainting_lama_mpe.ckpt', device='cpu')
        model.eval()
        if cuda :
            model = model.cuda()
        INPAINTING_MODEL = model

def dispatch(use_inpainting: bool, use_poisson_blending: bool, cuda: bool, img: np.ndarray, mask: np.ndarray, inpainting_size: int = 1024, model_name: str = 'default') -> np.ndarray :
